...
- `left_edge`：对应主界面的“运行普通画线”，保持旧版左缘规则。适合词头缩进稳定、正文不会顶到栏左缘的版式。
- `paddleocr`：对应“运行OCR画线”，适合词头可由文字形式、字号、粗体或特殊符号区分的版式。它会填入词头文字并生成坐标，建议先用典型页调整正则和阈值，再做批量画线。

v2.4 已移除 `projection` / `hybrid` 及投影空白高度、投影阈值系数、自适应阈值块、自适应阈值C。旧项目若保存了这两种检测方式，载入时自动回退到 `left_edge`。

### 倾斜和局部变形跟踪

“跟随词头列倾斜/变形”默认关闭，按需开启。算法在每栏预设起点左右的有限走廊中工作，不分析整页内容；每个纵向分块估计一次文字左边缘，再限制异常跳变并连接成分段路径。

- “列跟踪搜索半径”：允许词头列相对预设栏起点左右偏移的最大范围。
- “列跟踪分块高度”：每隔多高重新估计一次X位置。数值越小越能跟随局部弯曲，但也更容易受个别字符影响。
- “列跟踪最大步移”：相邻分块之间允许的最大横向变化，用于抑制污点、插图和正文造成的突然跳跃。

如果页面完全规整，或自动路径明显受到装饰线影响，可在页面列表上方关闭该选项框，恢复固定竖直检测带。

版面行为中的【手动分栏】与【跟随词头列倾斜和局部变形】均为默认关闭、按需开启。旧项目首次进入 v2.13.1 时会执行一次性默认值迁移；迁移后用户自己重新勾选的选择会正常保存。
...
- `AI 词头提取正则`：第一个捕获组作为原始词头。默认支持 `a·ga·rrón` 一类音节分隔形式，并兼容 OCR 把分隔点识别成 `.`, `:`, `-` 的常见情况。
- `AI 词性提示正则/词性搜索字符数`：在词头后的短距离内寻找 `s.m.`、`s.f.`、`adj.`、`v.`、`v.prnl.` 等标签；不会把定义正文中较后的词性缩写当成词头证据。
...
- `AI 特殊符号正则`：只标识可作为词条证据的项目符号；本词典说明为词条内部结构的 `■`、`□`、`||`、`~`、`→` 已由 dictionary profile 单独处理，不会触发新 lemma。
- `Paddle检测同时运行Tesseract对照`：对相同的拉直候选带额外运行 Tesseract，仅用于第二意见和诊断；沿用“OCR语言”和“Tesseract路径”。
...
- 提供两种主画线方式：`left_edge`（普通画线，使用旧版左缘排版规则）和 `paddleocr`（OCR识别词头与坐标并直接画线）。v2.4 已移除旧的 `projection` / `hybrid` 方式及其四个投影参数。
...
- 主界面取消菜单栏/顶部工具栏；常用版面参数、OCR参数、辅助显示与主要操作集中在左栏。页面列表占用左栏剩余高度并可滚轮滚动；完整参数仍可从“更多参数”打开。
